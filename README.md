...
A "The Source Library" section (open to every signed-in user, not admin-only) shows real photos and background on the classical Ayurvedic texts this app's recipes cite — `server/data/sourceTexts.json`, served via `GET /api/source-texts`.

- **Real photos, verified before use** — two (Charaka Samhita, Sushruta Samhita) are genuine manuscript folio photos from the Wellcome Collection. The other four (Ashtanga Hridaya, Sharangdhara Samhita, Bhaisajya Ratnavali, Bhavaprakasha) have no manuscript photo on Wikimedia Commons, so a historical printed edition's title page is shown instead — each card says so explicitly (`isManuscriptPhoto: false`) rather than implying it's an ancient manuscript.
- **"N recipes reference this text"** on each card is computed live from the current recipe database (`matchKeywords` matched against each recipe's `source` field) — not a hand-maintained count.
- **Fully translated**, including the `era` field — generated by `server/scripts/generateSourceTextTranslations.js` (re-runnable, same resumable pattern as the recipe translation script).
- **Reused in the weekly digest** — `digest.js` matches that day's Recipe of the Day against this same data and includes the relevant scripture photo in the email, tying the two features together.
